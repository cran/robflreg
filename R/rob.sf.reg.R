rob.sf.reg <-
function(Y, X, emodel = c("classical", "robust"), fmodel = c("LTS", "MM", "S", "tau"),
    ...
    if(!is.null(nbasis)){
      ...
      if(length(nbasis) != length(X))
        stop("Error!! The lengths of X and nbasis mut be equal!")
    }
    if(!is.null(ncomp)){
      if(!is.numeric(ncomp))
    ...
    if(is.null(nbasis)) nbasis = rep(20, np)
    if(is.null(gp)){
      gp <- list()
      for(iip in 1:np)
        gp[[iip]] = seq(0, 1, length.out = px[iip])
    }
    if(is.null(ncomp)) ncomp = rep(4, np)
    ...
    Bhat <- est.sf.fun(Y = Y, sco_X = do.call(cbind, sco_X),
                    emodel = emodel, fmodel = fmodel)
    
    Yfit <- do.call(cbind, sco_X) %*% Bhat + mean_Y
    resids <- Y - Yfit
